...
set -euo pipefail
DASHBOARD_HOSTNAME="{{ include "tyk-dashboard.proto" . }}://{{ include "dashboard.fullname" . }}.{{ .Release.Namespace }}.svc.cluster.local:{{ .Values.containerPort }}"
...
if [ "$DASHBOARD_ENABLED" = "true" ] && [ "$BOOTSTRAP_DASHBOARD" = "true" ]; then
  main $DASHBOARD_HOSTNAME
fi

kubectl create secret -n {{ .Release.Namespace }} generic tyk-operator-conf \
...
if [ "$DASHBOARD_ENABLED" = "true" ]; then
  ...
  kubectl rollout restart deployment/{{ include "dashboard.fullname" . }}
fi
