...
if [ "$ENTERPRISEPORTAL_SECRET_ENABLED" = "true" ] && [ "$BOOTSTRAP_DASHBOARD" = "true" ]
then
  kubectl get secrets -n "$TYK_POD_NAMESPACE" "$ENTERPRISEPORTAL_SECRET_NAME" || enterprise_portal_secret_exists=$?

  # if $ENTERPRISEPORTAL_SECRET_NAME exists, delete the previous secret.
  if [[ $enterprise_portal_secret_exists -eq 0 ]]
  then
    kubectl delete secret -n "$TYK_POD_NAMESPACE" "$ENTERPRISEPORTAL_SECRET_NAME"
  fi
  
  kubectl create secret -n "$TYK_POD_NAMESPACE" generic "$ENTERPRISEPORTAL_SECRET_NAME" \
    --from-literal "TYK_AUTH=${USER_AUTH_CODE}" \
    --from-literal "TYK_ORG=${ORGID}"
fi
