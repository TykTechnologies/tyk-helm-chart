...
if kubectl get secret -n "$TYK_POD_NAMESPACE" "$ENTERPRISEPORTAL_SECRET_NAME"; then
    echo "$ENTERPRISEPORTAL_SECRET_NAME exists in $TYK_POD_NAMESPACE namespace, deleting $ENTERPRISEPORTAL_SECRET_NAME"
    kubectl delete secret -n "$TYK_POD_NAMESPACE" "$ENTERPRISEPORTAL_SECRET_NAME"
else
    echo "$ENTERPRISEPORTAL_SECRET_NAME does not exists in $TYK_POD_NAMESPACE namespace, skipping delete operation."
fi
